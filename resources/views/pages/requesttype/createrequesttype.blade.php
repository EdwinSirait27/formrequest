...
@section('subtitle', 'Create Request Type')
@section('content')
<style>
/* LIGHT MODE */
    ...
    <div class="px-4 space-y-6 pb-8">
        <div class="bg-gradient-to-r from-blue-500/10 to-cyan-500/10 border border-blue-500/30 rounded-2xl p-4">
            <div class="flex items-start space-x-3">
                ...
                <div class="flex-1">
                    <h3 class="text-sm font-semibold text-blue-400 mb-1">Tips for Making Request Type</h3>
                    <p class="text-xs text-slate-400 leading-relaxed">For Request Type's name must be unique okay.</p>
        ...
        <form method="POST" action="{{ route('storerequesttype') }}" 
            class="space-y-5">
            {{-- <form  method="POST" action="{{ route('ticketreq') }}" enctype="multipart/form-data" class="space-y-5"> --}}
            @csrf
           
            <div>
                <label for="requesttype" class="block text-sm font-semibold text-slate-300 mb-2 flex items-center space-x-2">
