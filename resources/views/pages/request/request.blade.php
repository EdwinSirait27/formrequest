...
@section('content')
    ...
    render: function (data, type, row) {
        ...
        return 'Rp ' + new Intl.NumberFormat('id-ID', {
            ...
            maximumFractionDigits: 2
                        ...
                        //         } else if (status === 'Approved Director') {
                        //             badgeClass = 'bg-green-500 text-white';
                        //         }
                        //         else if (status === 'Submitted') {
                        //             badgeClass = 'bg-yellow-500 text-white';
                        //         } 
                        //         else if (status === 'Rejected Director') {
                        //             badgeClass = 'bg-yellow-500 text-white';
                        //         } 
                        //         else if (status === 'Rejected Manager') {
    ...
    render: function(data, type, row) {
        ...
        const labelMap = {
            'Draft'             : 'Draft',
            'Submitted'         : 'Submitted',
            'Approved Manager'  : 'Appr. Manager',
            'Rejected Manager'  : 'Rej. Manager',
            'Approved Finance'  : 'Appr. Finance',
            'Rejected Finance'  : 'Rej. Finance',
            'Approved Director' : 'Appr. Director',
            'Rejected Director' : 'Rej. Director',
            'Done'              : 'Done',
        };
        ...
        const badgeMap = {
            ...
            'Approved Manager'  : 'bg-green-600 text-white',
            'Rejected Manager'  : 'bg-red-500 text-white',
            'Approved Finance'  : 'bg-green-700 text-white',
            'Rejected Finance'  : 'bg-red-600 text-white',
            'Approved Director' : 'bg-green-500 text-white',
            'Rejected Director' : 'bg-yellow-500 text-white',
            'Done'              : 'bg-blue-500 text-white',
        };

        let label      = labelMap[status] ?? status;
        let badgeClass = badgeMap[status] ?? 'bg-slate-500 text-white';

        return `<span class="px-3 py-1 rounded-full text-xs font-semibold ${badgeClass}">${label}</span>`;
    }
},
                        {
                            data: 'action',
