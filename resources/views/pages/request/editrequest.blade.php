        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" id="title" name="title"
                    ...
                    placeholder="butuh segera untuk bla bla bla" required>
                @error('title')
                    <p class="mt-1.5 text-xs text-red-400 flex items-center gap-1">
                        <svg class="w-3.5 h-3.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                            <path fill-rule="evenodd"
        ...
        <div>
                ...
                {{-- <select id="status" name="status"
                    ...
                    @foreach ($statuses as $id => $name)
                        <option value="{{ $id }}"
                            {{ old('status', $request->statuses) == $id ? 'selected' : '' }}>
                            {{ $name }}
                        </option>
                    @endforeach
                </select> --}}
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        flatpickr("#request_date", { dateFormat: "Y-m-d", allowInput: true });
...
<script>
    ...
    $(document).ready(function () {
        ...
        // Init select2 setelah semua row ter-render
        $('.select2').select2({ placeholder: "Choose", allowClear: true, width: '100%' });
    });
</script>
