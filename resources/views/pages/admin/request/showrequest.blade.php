...
<div class="px-4 pb-8">
    ...
    {{-- <form method="POST" action="{{ route('updaterequest', $request->id) }}" class="space-y-5"> --}}
        <div class="space-y-4">
            <div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                ...
                <input type="date" id="request_date" name="request_date"
                    class="form-input w-full px-4 py-3 rounded-xl"
                    value="{{ old('request_date', $request->request_date) }}" disabled>
                @error('request_date')
                    <p class="mt-1.5 text-xs text-red-400 flex items-center gap-1">
                        <svg class="w-3.5 h-3.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                            <path fill-rule="evenodd"
            ...
            <div>
                ...
                <input type="date" id="deadline" name="deadline"
                    class="form-input w-full px-4 py-3 rounded-xl"
                    value="{{ old('deadline', $request->deadline) }}" disabled>
                @error('deadline')
                    <p class="mt-1.5 text-xs text-red-400 flex items-center gap-1">
                        <svg class="w-3.5 h-3.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                            <path fill-rule="evenodd"
                        ...
                        <tr>
                            ...
                            <td class="p-2">
                                <select name="items[{{ $i }}][uom]"
                                    class="select2 w-full form-input rounded-lg px-2 py-1" disabled>
                                    @foreach ($uoms as $uom)
                                        <option value="{{ $uom }}"
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        flatpickr("#request_date", { dateFormat: "Y-m-d", allowInput: false });
        flatpickr("#deadline", { dateFormat: "Y-m-d", allowInput: false });
    });
</script>

<script>
    $(document).ready(function () {
        $('#request_type_id').select2({ placeholder: "Choose request Types...", allowClear: true, width: '100%' });
        $('#vendor_id').select2({ placeholder: "Choose Vendors...", allowClear: true, width: '100%' });
