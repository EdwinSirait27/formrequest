    ...
    <div class="max-w-3xl mx-auto space-y-6">
        ...
        <div class="rounded-2xl border p-6 shadow-sm transition-colors duration-300"
            ...
            <div class="flex items-center gap-5 pb-6 mb-6 border-b" style="border-color: var(--border-color);">
                ...
                <div class="flex-shrink-0 w-16 h-16 rounded-2xl shadow-lg shadow-blue-500/30 overflow-hidden">

    @if (Auth::user()->employee->photos)
        <img src="{{ \Storage::url(Auth::user()->employee->photos) }}"
            alt="{{ Auth::user()->employee->employee_name }}"
            class="w-full h-full object-cover">
    @else
        <div class="w-full h-full flex items-center justify-center text-white font-bold text-xl bg-blue-500">
            {{ strtoupper(
...
<div class="space-y-1.5">
                ...
                <div class="space-y-1.5">
                    ...
                    <div class="flex items-center gap-3 px-4 py-3 rounded-xl border transition-colors"
                        ...
                        <span class="text-sm font-medium" style="color: var(--text-primary)">
                            {{ Auth::user()->employee->grading->grading_name ?? '-' }}
                        </span>
                    </div>
                </div>
    ...
    @if (auth()->user()->employee->signature)
        <div class="border rounded-xl p-4 bg-gray-50">
            <p class="text-sm text-gray-500 mb-3">Current signature</p>
            <div class="flex justify-center">
               <img src="{{ \Storage::url(auth()->user()->employee->signature) }}"
    class="h-24 object-contain">
            </div>
        </div>
    @endif
...
<div class="space-y-1.5 sm:col-span-2">
    ...
    <div class="flex items-center justify-between gap-4 px-4 py-4 rounded-xl border transition-colors"
        style="background-color: var(--bg-secondary); border-color: var(--border-color);">
            ...
            <div class="flex items-center gap-4 w-full">
                <div class="flex-shrink-0 w-10 h-10 rounded-lg flex items-center justify-center"
                    ...
                    <svg class="w-5 h-5" style="color: var(--text-muted)" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M15.232 5.232l3.536 3.536M9 13l6.586-6.586a2 2 0 112.828 2.828L11.828 15.828a4 4 0 01-1.414.93l-3 1 1-3a4 4 0 01.93-1.414z" />
                    </svg>
                </div>
                <div class="flex-1 min-w-0">
                    <p class="text-xs mb-2" style="color: var(--text-muted)">Current signature</p>
                    <div class="rounded-lg overflow-hidden border px-4 py-2 inline-flex"
                        style="background-color: var(--bg-card); border-color: var(--border-color);">
                        <img src="{{ \Storage::url(auth()->user()->employee->signature) }}"
        ...
        @else
            <div class="flex items-center gap-3 w-full">
                ...
                <div>
                    <p class="text-sm font-medium" style="color: var(--text-secondary)">No signature yet</p>
                    <p class="text-xs" style="color: var(--text-muted)">Contact your administrator to add a signature</p>
                </div>
            </div>
        @endif
