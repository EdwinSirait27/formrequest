...
public function update(Request $request, $hash)
{
   ...
   $request->validate([
    ...
    // $roles = $request->roles ?? [];
        $roles = $request->roles; // ← hapus ?? []


    // Simpan semua role ke all_roles_bdtix
    $user->update(['all_roles_formrequest' => $roles]);

    // Cek active_role_bdtix masih ada di role baru
    $activeRole = $user->active_role_formrequest;
    if (!in_array($activeRole, $roles)) {
        $activeRole = $roles[0] ?? null;
        $user->update(['active_role_formrequest' => $activeRole]);
    }
...
public function bulkUpdateRole(Request $request)
{
    ...
    $users->each(function ($user) {
        $user->update([
            'all_roles_formrequest'   => ['user'],
            'active_role_formrequest' => 'user',
        ]);
    ...
    public function edit($hash)
{
    ...
    $roles = Role::where('guard_name', 'web')->get();
    $userRoles = $user->getRoleNames()->toArray();
    $allRolesFormrequest = $user->all_roles_formrequest ?? []; // ← tambah ini

    return view('pages.users.editusers', compact('user', 'roles', 'userRoles', 'allRolesFormrequest'));
}
}
